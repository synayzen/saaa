...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  message.channel.send(embed.setDescription(`<:micack:843105475175120946> ${uye} üyesinin, ${message.author} tarafından mutesi kaldırıldı!`)).catch();
  if(ayar.muteLogKanali && client.channels.cache.has(ayar.muteLogKanali)) client.channels.cache.get(ayar.muteLogKanali).send(embed.setDescription(`<:micack:843105475175120946> **Mutele Kaldırıldı!** \n ${uye} (\`${uye.id}\`) \n **Yetkili:** ${message.author} (\`${message.author.id}\`) tarafından \n **mutesi kaldırıldı!**`)).catch();
};
