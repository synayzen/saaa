...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  message.channel.send(embed.setDescription(`<:muteli:842034165918728212> ${uye} üyesi, ${message.author} tarafından **${sure}** boyunca **${reason}** nedeniyle seste mutelendi!`)).catch();
  if(ayar.muteLogKanali && client.channels.cache.has(ayar.muteLogKanali)) client.channels.cache.get(ayar.muteLogKanali).send(embed.setDescription(`<:muteli:842034165918728212> **Seslide Mutelendirildi** \n **Seste Mutelenilen:** ${uye} (\`${uye.id}\`) \n **Yetkili:** ${message.author} (\`${message.author.id}\`) \n **Süre:** \`${sure}\` \n **Sebep:** \`${reason}\` nedeniyle seste mutelendi!`)).catch();
};
