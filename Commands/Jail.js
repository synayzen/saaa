...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  message.channel.send(embed.setDescription(`<:jaill:843074433052049438> ${uye} (\`${uye.id}\`) üyesi , \n ${message.author} (\`${message.author.id}\`) tarafından **${reason}** nedeniyle jaile atıldı!`)).catch();
  if(ayar.jailLogKanali && client.channels.cache.has(ayar.jailLogKanali)) client.channels.cache.get(ayar.jailLogKanali).send(embed.setDescription(`<:jaill:843074433052049438> **Cezalandırıldı!** \n **Cezalandırılan**:  ${uye} (\`${uye.id}\`)  \n **Yetkili:** ${message.author} (\`${message.author.id}\`) \n **Sebep:** \`${reason}\` nedeniyle jaile atıldı! `)).catch();
};
