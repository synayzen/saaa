...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(ayar.jailLogKanali && client.channels.cache.has(ayar.jailLogKanali)) client.channels.cache.get(ayar.jailLogKanali).send(embed.setDescription(`<:jailbitti:843074418427035679> Cezası Bitirildi ! \n **Cezalandırılan:** ${uye} (\`${uye.id}\`) \n **Kullanıcı:** ${message.author} (\`${message.author.id}\`) tarafından \n **jailden çıkarıldı!**`)).catch();
};
