...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if (!victim || !reason) return message.channel.send(embed.setDescription("Allahına kavuşturmak istediğin kişiyi etiketle ve sebebi de yaz")).then(x => x.delete({timeout: 5000}));
  ...
  if(ayar.banLogKanali && client.channels.cache.has(ayar.banLogKanali)) client.channels.cache.get(ayar.banLogKanali).send(new MessageEmbed().setColor("#9900ff").setTimestamp().setFooter("⚔ Assassins Developed by Synayzen İvar").setTitle('❌ Üye Kicklendi!').setDescription(`**Kickleyen Yetkili:** ${message.author} (\`${message.author.id}\`)\n **Kicklenen Üye:** ${victim.user.tag} (\`${victim.user.id}\`) \n**Sebep:** ${reason}`));
};
