...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  message.channel.send(new MessageEmbed().setColor(client.randomColor()).setTitle("Kayıt Sistemi").setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048})).setFooter("⚔ Assassins Developed by Synayzen İvar").setTimestamp().setDescription(`<a:yeil:838420699374682113> Kayıt Olan Kullanıcı: ${uye}\n <a:yeil:838420699374682113> Verilen Roller: ${uye.roles.cache.filter(a => a.name !== "@everyone").map(x => x).join(', ')}\n <a:yeil:838420699374682113> Kayıt Eden: ${message.author}\n <a:yeil:838420699374682113> Güncellenen İsim: \`${yazilacakIsim}\``)).catch();
};
