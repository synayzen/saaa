...
module.exports.execute = (client, message, args, ayar, emoji) => {
  ...
  const embed = new MessageEmbed().setTimestamp().setColor("#9900ff").setAuthor(message.guild.name, message.guild.iconURL({ dynamic: true })).setFooter("⚔ Assassins Developed by Synayzen İvar")
  .setDescription(`**Toplam Üye:** ${message.guild.memberCount}\n**Aktif Üye:** ${message.guild.members.cache.filter(u => u.presence.status != "offline").size}\n**Kanallar:** ${message.guild.channels.cache.size} (${message.guild.channels.cache.filter(c => c.type === "text").size} Metin kanalı, ${message.guild.channels.cache.filter(c => c.type === "voice").size} Ses kanalı )\n**Roller:** ${message.guild.roles.cache.size}\n**Oluşturulma Tarihi:** ${moment(message.guild.createdAt).format('DD/MM/YYYY | HH.mm.ss')}`);
