...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(!ayar.banciRolleri.some(rol => message.member.roles.cache.has(rol)) && !message.member.roles.cache.has(ayar.sahipRolu)) return message.channel.send(embed.setDescription("Heyy! Bu komutu kullanabilmek için gerekli rollere sahip değilsin dostum!")).then(x => x.delete({timeout: 5000}));
  ...
  if (!reason) return message.channel.send(embed.setDescription("Allahına kavuşturmak istediğin kişiyi etiketle ve sebebi de yaz")).then(x => x.delete({timeout: 5000}));
  if (!victim) {
    ...
    if(kisi) {
      ...
      if(ayar.banLogKanali && client.channels.cache.has(ayar.banLogKanali)) client.channels.cache.get(ayar.banLogKanali).send(new MessageEmbed().setColor("#9900ff").setTimestamp().setFooter("⚔ Assassins Developed by Synayzen İvar").setTitle('Üye Banlandı!').setDescription(`**Banlayan Yetkili:** ${message.author} (${message.author.id})\n**Banlanan Üye:** ${kisi.tag} (${kisi.id})\n**Sebep:** ${reason}`));
    } else {
      message.channel.send(embed.setDescription("Allahına kavuşturmak istediğin kişiyi etiketle ve sebebi de yaz")).then(x => x.delete({timeout: 5000}));
    };
    return message.reply("Allahına kavuşturmak istediğin kişiyi etiketle ve sebebi de yaz!").then(x => x.delete({timeout: 5000}));
  };
  ...
  message.channel.send(embed.setImage("https://cdn.discordapp.com/attachments/801730953813688340/803663058366103562/orjin.gif").setDescription(`\`${victim.user.tag}\` üyesi ${message.author} tarafından **${reason}** nedeniyle **banlandı!**`));
